...
```{r}
cohensd_2 = function(diff_,g1,g2,dat,svd){
   g1 = dat %>% filter(Role == .data[[g1]]) %>% select(.data[[svd]])
  #g2 = dat %>% filter(Role == .data[[g2]]) %>% select(.data[[svd]])
  
  #d = diff_/(sqrt((sd(g1)^2 + sd(g2)^2)/2))
  #return(d)
   return(g1)
}
```
